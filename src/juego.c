...
int cargar_juego(void) {
  ...
  lifes = sfText_create();
  sfText_setString(lifes, "Lifes 3");
  sfText_setCharacterSize(lifes, 20);
  sfText_setPosition(lifes, (sfVector2f){50,550});
  sfText_setFont(lifes, sfFont_createFromFile("resources/232MKRL_.TTF"));
  sfText_setColor(lifes, sfBlack);

  //fix
  gameover = sfText_create();
  sfText_setString(gameover, "GAME OVER");
  sfText_setCharacterSize(gameover, 60);
  sfText_setPosition(gameover, (sfVector2f){260,330});
  sfText_setFont(gameover, sfFont_createFromFile("resources/232MKRL_.TTF"));
...
int dibujar_juego(sfRenderWindow* w){
  ...
  asprintf(&string, "Lifes %d", vidas);
  sfText_setString(lifes, string);
  free(string);
  sfRenderWindow_drawText(w, lifes, NULL);
